...
const getAllCommentsForEntity = async function(entityType, entityObj) {
    ...
    switch(entityType)
 {
     case "GOAL": entityQuery = `SELECT c.comment_id, c.date_time::time, c.message, tm.first_name, tm.last_name, tm.avatar
                ...
                break;
    case "USER": entityQuery = `SELECT c.comment_id, c.date_time::time, c.message, tm.first_name, tm.last_name, tm.avatar
                                FROM team_member AS tm
                ...
                break;
    case "TEAM": entityQuery = `SELECT c.comment_id, c.date_time::time, c.message, tm.first_name, tm.last_name, tm.avatar
                                FROM team_member AS tm
                                        INNER JOIN comment as c on c.member_id = tm.member_id
                                WHERE c.goal_id IS NULL
                                AND DATE(c.date_time) = $1
